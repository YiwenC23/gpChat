...
function reset_data(): {
    ...
    for (const [, stream_data] of streams_dict) {
        if (stream_data.folder_id === OTHER_CHANNELS_FOLDER_ID) {
            const other_channels_folder = channel_folders_dict.get(OTHER_CHANNELS_FOLDER_ID);
            if (other_channels_folder === undefined) {
                channel_folders_dict.set(OTHER_CHANNELS_FOLDER_ID, {
                    id: OTHER_CHANNELS_FOLDER_ID,
                    header_id: OTHER_CHANNEL_HEADER_ID,
                    name: $t({defaultMessage: "OTHER CHANNELS"}),
                    is_header_visible: !stream_data.is_hidden,
                    unread_count: stream_data.unread_count,
                    is_collapsed: collapsed_containers.has(OTHER_CHANNEL_HEADER_ID),
                    has_unread_mention: stream_data.mention_in_unread,
                });
            } else {
                other_channels_folder.unread_count =
                    (other_channels_folder.unread_count ?? 0) + (stream_data.unread_count ?? 0);
                other_channels_folder.is_header_visible =
                    other_channels_folder.is_header_visible || !stream_data.is_hidden;
                other_channels_folder.has_unread_mention =
                    other_channels_folder.has_unread_mention || stream_data.mention_in_unread;
            }
        } else if (stream_data.folder_id === PINNED_CHANNEL_FOLDER_ID) {
            const pinned_channel_folder = channel_folders_dict.get(PINNED_CHANNEL_FOLDER_ID);
            if (pinned_channel_folder === undefined) {
                channel_folders_dict.set(PINNED_CHANNEL_FOLDER_ID, {
                    id: PINNED_CHANNEL_FOLDER_ID,
                    header_id: PINNED_CHANNEL_HEADER_ID,
                    name: $t({defaultMessage: "PINNED CHANNELS"}),
                    is_header_visible: !stream_data.is_hidden,
                    unread_count: stream_data.unread_count,
                    is_collapsed: collapsed_containers.has(PINNED_CHANNEL_HEADER_ID),
                    has_unread_mention: stream_data.mention_in_unread,
                });
            } else {
                pinned_channel_folder.unread_count =
                    (pinned_channel_folder.unread_count ?? 0) + (stream_data.unread_count ?? 0);
                pinned_channel_folder.is_header_visible =
                    pinned_channel_folder.is_header_visible || !stream_data.is_hidden;
                pinned_channel_folder.has_unread_mention =
                    pinned_channel_folder.has_unread_mention || stream_data.mention_in_unread;
            }
        } else {
            const folder_id = stream_data.folder_id;
            let channel_folder = channel_folders_dict.get(folder_id);
            if (channel_folder === undefined) {
                channel_folder = {
                    id: folder_id,
                    header_id: CHANNEL_FOLDER_HEADER_ID_PREFIX + folder_id,
                    name: channel_folders.get_channel_folder_name_from_id(folder_id),
                    is_header_visible: !stream_data.is_hidden,
                    unread_count: stream_data.unread_count,
                    is_collapsed: collapsed_containers.has(
                        CHANNEL_FOLDER_HEADER_ID_PREFIX + folder_id,
                    ),
                    has_unread_mention: stream_data.mention_in_unread,
                };
                channel_folders_dict.set(folder_id, channel_folder);
            } else {
                channel_folder.unread_count =
                    (channel_folder.unread_count ?? 0) + (stream_data.unread_count ?? 0);
                channel_folder.is_header_visible =
                    channel_folder.is_header_visible || !stream_data.is_hidden;
                channel_folder.has_unread_mention =
                    channel_folder.has_unread_mention || stream_data.mention_in_unread;
            }
        }
    }

    if (channel_folders_dict.size === 1) {
        const other_channels_folder = channel_folders_dict.get(OTHER_CHANNELS_FOLDER_ID);
        if (other_channels_folder !== undefined) {
...
function bulk_insert_channel_folders(channel_folders: Set<number>): void {
    ...
    for (const [folder_id, channel_folder] of channel_folders_dict) {
        if (channel_folders.has(folder_id)) {
            const $channel_folder = render_inbox_folder_with_channels({
                ...channel_folder,
                topics_dict,
                streams_dict,
            });
...
export function update(): void {
    ...
    for (const [folder_id, folder_info] of folders_info.entries()) {
        ...
        if (folder_id === PINNED_CHANNEL_FOLDER_ID) {
            name = $t({defaultMessage: "PINNED CHANNELS"});
        } else if (folder_id === OTHER_CHANNELS_FOLDER_ID) {
            name = $t({defaultMessage: "OTHER CHANNELS"});
        } else {
            name = channel_folders.get_channel_folder_name_from_id(folder_id);
        }
